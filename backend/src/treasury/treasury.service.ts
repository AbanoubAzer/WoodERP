...
export class TreasuryService {
  ...
  async createTransfer(companyId: string, data: any) {
    return this.prisma.$transaction(async (prisma) => {
      const fromAcc = await prisma.treasuryAccount.findFirst({ where: { id: data.fromAccountId, companyId }});
      const toAcc = await prisma.treasuryAccount.findFirst({ where: { id: data.toAccountId, companyId }});

      if (!fromAcc || !toAcc) throw new BadRequestException('Account not found');
      
      const amount = parseFloat(data.amount);
      if (amount <= 0) throw new BadRequestException('Amount must be positive');
      if (fromAcc.balance < amount) throw new BadRequestException('Insufficient balance in source account');
      ...
      await prisma.treasuryTransaction.create({
        data: { accountId: fromAcc.id, type: 'TRANSFER', amount: -amount, description: data.description }
      });
      ...
      await prisma.treasuryTransaction.create({
        data: { accountId: toAcc.id, type: 'TRANSFER', amount: amount, description: data.description }
      });
