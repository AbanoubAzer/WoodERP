  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || 'حدث خطأ أثناء إضافة المورد');
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center space-x-3 space-x-reverse bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">تسجيل مورد جديد</h1>
          <p className="text-sm text-slate-500">إدخال البيانات الأساسية، المالية، والأرصدة المستحقة</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">اسم المورد / المسؤول (مطلوب)</label>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">تصنيف المورد</label>
                <SearchableSelect
                  options={[
                    { value: 'LOCAL', label: 'مورد أخشاب محلي' },
                    { value: 'IMPORTED', label: 'مستورد / خارجي' },
                    { value: 'MDF', label: 'مورد MDF وألواح' },
                    { value: 'ACCESSORIES', label: 'مورد إكسسوارات ومواد أخرى' }
                  ]}
              ...
              <div>
                ...
                <SearchableSelect
                  options={[
                    { value: 'CASH', label: 'دفع نقدي فوري' },
                    { value: '30_DAYS', label: 'آجل 30 يوم' },
                    { value: '60_DAYS', label: 'آجل 60 يوم' }
                  ]}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">بيانات الحساب البنكي</label>
                <input type="text" placeholder="اسم البنك، رقم الحساب أو الآيبان..." value={formData.bankDetails} onChange={e => setFormData({...formData, bankDetails: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-[var(--color-brand-primary)]" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">الرصيد الافتتاحي (مستحقات حالية للمورد)</label>
                <input type="number" min="0" value={formData.openingBalance} onChange={e => setFormData({...formData, openingBalance: parseFloat(e.target.value) || 0})} className="w-full px-3 py-2 border rounded-lg focus:ring-[var(--color-brand-primary)] bg-amber-50" />
                <p className="text-xs text-slate-500 mt-1">المبالغ المستحقة للمورد قبل استخدام النظام</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-6 border-t border-slate-100">
            ...
            >
              ...
              <span>{isSubmitting ? 'جاري الحفظ...' : 'حفظ بيانات المورد'}</span>
            </button>
          </div>
        </form>
      </div>
